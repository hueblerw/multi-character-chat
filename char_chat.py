...
config_list = [
    {
        ...
        "base_url": "https://jwn-pocs-sandbox.openai.azure.com/",
...
character_task = "Speak conversationally as a character with the description supplied below.  Send what your character would speak in response to the prompts received by the chat manager and the conversation spoken by the other characters.  Each response is a single reply consisting of what your character would say and / or what actions they would take.  When speaking address one character or one group of characters.  Place actions in italics."
...
def handle_messages(recipient, messages, sender, config):
    ...
    remembered_messages = messages

    if (len(messages) > MAX_MESSAGES):
        remembered_messages = messages[-MAX_MESSAGES:]

    print(f"The number of messages used is {len(remembered_messages)}")
    print(f"messages content: {remembered_messages[-1]['content']}")

    if all(key in remembered_messages[-1] for key in ["name"]):
        chat_interface.send(
            remembered_messages[-1]["content"], user=remembered_messages[-1]["name"], respond=False
        )
    else:
        chat_interface.send(remembered_messages[-1]["content"], user="user_proxy", respond=False)
...
groupchat = autogen.GroupChat(
    agents=[wilson_von_hamil, selena_russo, renata_cyzon, anwyn, zoana, user_proxy],
    messages=[],
